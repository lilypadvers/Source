...
public class BlockFlowing extends BlockFluid {
		...
		if(var6 > 0) {
			...
			if(this.numAdjacentSources >= 2 && this.material == Material.water) {
				if(worldObj.g(x, y - 1, z)) {
					var9 = 0;
				} else if(worldObj.f(x, y - 1, z) == this.material && worldObj.e(x, y, z) == 0) {
					var9 = 0;
				}
			}
		...
		if(this.liquidCanDisplaceBlock(worldObj, x, y, z)) {
			...
			if(var6 > 0) {
				if(this.material == Material.lava) {
					...
					Block.blocksList[var6].dropBlockAsItem(worldObj, x, y, z, worldObj.e(x, y, z));
				}
			}
	...
	private int calculateFlowCost(World worldObj, int x, int y, int z, int var5, int var6) {
		...
		for(int var8 = 0; var8 < 4; ++var8) {
			if((var8 != 0 || var6 != 1) && (var8 != 1 || var6 != 0) && (var8 != 2 || var6 != 3) && (var8 != 3 || var6 != 2)) {
				...
				if(!this.blockBlocksFlow(worldObj, var9, y, var11) && (worldObj.f(var9, y, var11) != this.material || worldObj.e(var9, y, var11) != 0)) {
	...
	private boolean[] getOptimalFlowDirections(World worldObj, int x, int y, int z) {
		...
		for(var5 = 0; var5 < 4; ++var5) {
			...
			if(!this.blockBlocksFlow(worldObj, var6, y, var8) && (worldObj.f(var6, y, var8) != this.material || worldObj.e(var6, y, var8) != 0)) {
	...
	private boolean liquidCanDisplaceBlock(World worldObj, int x, int y, int z) {
		Material var5 = worldObj.f(x, y, z);
