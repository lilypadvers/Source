...
public class ItemHoe extends BlockLeavesBase {
	protected float[] f = new float[320];
	protected float[] g = new float[320];

	public ItemHoe(int var1) {
		super(ln.ar.aZ + var1 * 16);
	}
	...
	public void a() {
		...
		for(int var1 = 0; var1 < 16; ++var1) {
			for(var2 = 0; var2 < 20; ++var2) {
				...
				this.g[var1 + var2 * 16] = var4 / ((float)var3 * 1.06F);
				if(var2 >= 19) {
					this.g[var1 + var2 * 16] = (float)(Math.random() * Math.random() * Math.random() * 4.0D + Math.random() * (double)0.1F + (double)0.2F);
				}
			}
		}
